...
func resume()->bool:
	...
	while executionState == YarnGlobals.ExecutionState.Running:
		...
		if _state.programCounter >= _currentNode.instructions.size():
			var completeInstruction = nodeCompleteHandler.call_func(_currentNode.nodeName)
			run_instruction(completeInstruction)
			executionState = YarnGlobals.ExecutionState.Stopped
			reset()
...
func run_instruction(instruction)->bool:
	...
	match instruction.operation:
		...
		YarnGlobals.ByteCode.RunCommand:
				...
				#DEBUG
				print("DEBUG: in vm RunCommand case. Operands:")
		...
		YarnGlobals.ByteCode.ShowOptions:
			...
			if _state.currentOptions.size() == 0:
				print("DEBUG: _state.currentOptions.size() == 0")
				executionState = YarnGlobals.ExecutionState.Stopped
				reset()
				dialogueCompleteHandler.call_func()
				return false
			print("DEBUG: _state.currentOptions.size() != 0 and here's the currentOptions array:")
			print(_state.currentOptions)
